...
var Rental = Backbone.Model.extend({
  sync: function(method, model, options) {
    ...
    console.log("model.collection: ", model.collection);
     if (method == "create") {
       options.url = 'http://localhost:3000/rentals/' + model.attributes.title + "/check-out?customer_id=" + model.attributes.customer_id + "&checkout_date=" + model.attributes.checkout_date + "&due_date=" + model.attributes.due_date;
     }
